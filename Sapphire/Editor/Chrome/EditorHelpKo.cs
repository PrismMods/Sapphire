...
namespace Sapphire
{
    /* Korean help translations, kept in ONE place so they're easy to edit without touching the
       ...
       (or a null value) simply falls back to English.

       Bodies are verbatim (@"…") literals: what you see here is what the panel shows — real
       line breaks, a blank line for a paragraph break, <b>…</b> for bold, and "" for a literal
       double quote. Entries follow EditorHelp's Index grouping so the two files line up. */
...
<b>단축키</b>
3 — 토글 (타일 미선택 시). 키 개수는 하위 메뉴에서 정합니다."),
...
<b>단축키</b>
4 — 토글 (타일 미선택 시). 키 개수는 하위 메뉴에서 정합니다."),
...
<b>단축키</b>
타일 미선택 시 숫자 1–0으로 도구 선택. 기본은 꺼져 있으며, 단축키 › 도구 모음 단축키 제거를 끄면 사용할 수 있습니다."),
...
<b>박자</b> — 전체 길이. 2박자 = 360° (기본값). 타일당 각도 = 박자×180/N (2박자 8키 = 45°)."),
            ...
            ["AnglePad"] = new KeyValuePair<string, string>("각도 패드",
@"<b>기능</b>
...
<b>버튼</b>
소용돌이 — 맨 첫 타일의 소용돌이를 <b>첫 회차에만</b> 반전합니다. 켜져 있으면 버튼이 강조됩니다.
+ — 패드를 복제하며 내용과 반복 횟수를 함께 가져갑니다(패드 하나가 임시 프리셋입니다).
× n — 표현식을 n번 놓습니다. (…)*n과 별개로 사용 가능합니다. 횟수로 두면 표현식이 하나의 단위로 남아, 맨 앞 소용돌이를 빼는 효과가 첫 회차에만 적용되고 회전 방향은 다음 회차로 이어집니다.
도형 라이브러리에 추가 — 반복까지 포함한 묶음을 '각도 패드에서 추가됨'에 저장합니다.
...
에디터 팝업의 Sapphire 스타일 버전입니다. 버튼은 게임 자체 버튼을 그대로 사용합니다."),
            ...
            // ── Timeline & camera / 타임라인 · 카메라 ───────────────────

            ["SapphireEditorEvents"] = new KeyValuePair<string, string>("타임라인",
@"<b>기능</b>
실제 곡 시간 기준의 이벤트 타임라인: 카테고리별 마커, 재생 헤드, 확대, 트랜스포트(재생/되감기 · 시계 · BPM), 모드 칩(EDIT·PLAY 전환 / 난이도 / NO FAIL / AUTO).
